...
function getProductInfoPriceDetailsByPostcode($p_id, $postCode) {
        $CI = & get_instance();
        $CI->db->select('product_price_by_city.*');
        $CI->db->from('product_price_by_city');
        $CI->db->join('users', 'users.super_city_id=product_price_by_city.super_city_id');
...
function view_amount_function($value=0){
    if($value > 0){
        $value_ = floatval(str_replace(',','.',$value));
        return CURRENCY_SYMBOL.number_format((float) $value_, 2, ',', ''); //number_format($value,2,",",",") ;
    }else{
        return CURRENCY_SYMBOL.number_format((float) 0, 2, ',', '');
    }
