...
class Vereniging_management extends MX_Controller {
    ...
    public function add_vereniging_by_city() {
        ...
        if ($_POST) {
            ...
            $slct_postcode = explode("|",$this->input->post('post_code'));
            
            if(!empty($slct_postcode[0])){
                $postcode = $slct_postcode[0] ;
            }

            if(!empty($slct_postcode[1])){
                $city_id = $slct_postcode[1] ;
            }
        ...
        $this->db->distinct()->select('country')->from('users')->where(array('usertype' => 'City'))->order_by('users.u_id', 'DESC');
        $data['country'] = $this->db->get();
...
### in mail association name ######
    ...
    public function getCityPostCode() {
        ...
        $data = array();
        $post_code = $this->custom_model->getCityPostcodeByCountry($_POST['country_name']);
        $html = '';
        foreach ($post_code as $dd) {
            $html .= '<option value="'.$dd->post_code.'|'.$dd->u_id.'">' . $dd->post_code .' - '. $dd->city_name  .'</option>';
        }
        echo $html;
    }
