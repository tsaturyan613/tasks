...
class Gemeenten_management extends MX_Controller {
    ...
    public function add_gemeenten() {
     ...
     public function add_gemeenten_using_csv() {
        $data = array();
        $this->load->view('gemeenten/add_gemeenten_using_csv', $data);
    }

    public function save_gemeenten_by_city_using_csv() {
        $count = 0;
            ...
            while ($csv_line = fgetcsv($fp, 1024)) {
                ...
                for ($i = 0, $j = count($csv_line); $i < $j; $i++) {
                        $insert_csv = array(
                            'country'   => $csv_line[0],
                            'city_name' => $csv_line[1],
                            'post_code' => $csv_line[2],
                            'usertype' => 'City',
                            'added_by' => 'Admin',
                            'u_status' => 'Y',
                        );
                ...
                $i++;
                if($insert_csv['country'] !=''){
                    $this->common_model->AddRecord($insert_csv,'users');
                }
