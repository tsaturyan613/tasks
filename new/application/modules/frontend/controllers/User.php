...
//echo $this->db->last_query(); die;
    ...
    public function registerVerenigingen() {
        $slct_postcode = explode("|",$this->input->post('postcode'));
        $city_id = 0;
        $postcode = 0;
        ...
        $country_name = $this->common_model->RetriveRecordByWhereRow('users', array('country'=>$country_id));
        if(!empty($country_name)){
           $country_id =  $country_name['u_id'];
        } 

        $city_name = $this->common_model->RetriveRecordByWhereRow('users', array('u_id'=>$city_id));
        if(!empty($city_name)){
        ...
        if ($this->input->post('association') == 0) {
            ...
            $dd['country_name'] = $this->input->post('country_name');
            $dd['city_id'] = $city_id;
            $dd['post_code'] = $postcode;
